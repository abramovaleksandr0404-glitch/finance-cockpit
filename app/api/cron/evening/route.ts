...
export async function GET(req: Request) {
  ...
  try {
    ...
    for (const loan of loans ?? []) {
      ...
      else if (days === 2) alerts.push(`📅 Послезавтра платёж: ${loan.name} — ${rub(Number(loan.min_payment))}`)
    }

    // Переменные траты — порог 80%
    ...
    if (pctUsed >= 95) {
      alerts.push(`🔴 Переменные почти исчерпаны: *${pctUsed}%* (${rub(varSpent)} из ${rub(varBudget)}) — осталось ${rub(varBudget - varSpent)}`)
    } else if (pctUsed >= 80) {
