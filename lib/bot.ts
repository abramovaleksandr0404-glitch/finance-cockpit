...
8. РАЗЛИЧАЙ ТЕКУЩИЙ И СЛЕДУЮЩИЙ МЕСЯЦ:
   • «прогноз конца месяца» / «остаток к концу месяца» / «что будет 30-го» = ТЕКУЩИЙ месяц (июнь) → forecast_end из JSON
   • «прогноз на июль» / «следующий месяц» / «в июле» = СЛЕДУЮЩИЙ месяц → next_forecast из JSON
   • НИКОГДА не путай. Если неясно — уточни у пользователя.
