...
export const TOOLS = [
  { name:'add_expense', description:'Записать трату С ЛЮБОГО источника (дебет, наличные ИЛИ кредитная карта). ЕДИНСТВЕННЫЙ инструмент для трат — не комбинируй с mark_card_payment или update_cash, иначе трата запишется дважды. Обязательно укажи source; если пользователь не сказал откуда — спроси. Если анализируешь фото чека/выписки или пользователь называет дату в прошлом — ОБЯЗАТЕЛЬНО укажи date, иначе трата запишется сегодняшним числом и даты разъедутся.',
    input_schema:{type:'object',properties:{amount:{type:'number'},category:{type:'string',enum:['Еда и кафе','Транспорт','Здоровье','Развлечения','Одежда','Инвестиции','Прочее']},description:{type:'string'},date:{type:'string',description:'YYYY-MM-DD, если трата НЕ сегодня (дата на чеке/фото, или названа пользователем). Пропусти поле для сегодняшней даты.'},source:{type:'string',enum:['debit','cash','credit_tbank','credit_sber','split'],description:'Откуда списано: debit — Сбер дебет (уменьшает баланс); cash — наличные на руках (уменьшает наличные); credit_tbank / credit_sber / split — кредитная карта (растёт долг, дебет не трогается)'}},required:['amount','source']} },
  ...
  { name:'manage_recurring_income', description:'Добавить или убрать регулярный доход (стипендия, фриланс, аренда и т.п.) из списка ожидаемых поступлений. Используй на «у меня теперь есть Х каждый месяц» или «Х больше не будет».',
    input_schema:{type:'object',properties:{
      ...
      name:{type:'string',description:'Название дохода'},
      amount:{type:'number',description:'Сумма (для add)'},
      day:{type:'number',description:'День месяца поступления, 1-31 (для add)'},
    },required:['action','name']} },
  { name:'manage_debt_owed_to_me', description:'Кто-то должен деньги пользователю (не наоборот). Используй на «дал в долг X», «мне должны X», «вернули долг X».',
    input_schema:{type:'object',properties:{
      action:{type:'string',enum:['add','repaid'],description:'add — записать новый долг передо мной; repaid — отметить погашенным (полностью или частично)'},
      who:{type:'string',description:'Кто должен'},
      amount:{type:'number',description:'Сумма долга (add) или суммы возврата (repaid)'},
    },required:['action','who','amount']} },
