...
export const plannerTools = [
  ...
  {
    name: 'add_habit',
    description: 'ВСЕГДА вызывай когда упоминается создание привычки: «добавь привычку X», «новая привычка: X», «привычка: X», «хочу начать X», «буду делать X каждый день/неделю», «хочу X как привычку», «хочу медитировать/читать/бегать регулярно». Ключевые слова-триггеры: привычка, habit, регулярно, каждый день, каждое утро, N раз в неделю. НИКОГДА не отвечай «добавил/готово/принято» без реального вызова этого инструмента — пиши в БД, потом отвечай.' (зал, зарядка, медитация, чтение, бег, английский). Вызывай на «хочу начать ходить в зал», «добавь привычку медитировать», «буду читать каждый день», «хочу заниматься спортом 3 раза в неделю». НЕ для разовых дел (для них add_task). Привычки — это то, что повторяется и развивает; задачи — разовые поручения.',
    input_schema: {
      type: 'object',
      properties: {
